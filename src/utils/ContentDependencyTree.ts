...
export class ContentDependancyTree {
  ...
  constructor(items: RepositoryContentItem[]) {
    ...
    while (unresolvedCount > 0) {
      ...
      info = info.filter((item) => {
        const unresolvedDependancies = item.dependancies.filter(
          (dep) => !resolved.has(dep.dependancy.id as string)
        );
  ...
  private searchObjectForContentDependancies(
    ...
    if (Array.isArray(body)) {
      body.forEach((contained, index) => {
        this.searchObjectForContentDependancies(
          item,
          contained,
          result,
          body,
      ...
      if (
        ...
        result.push({
          dependancy: body as ContentDependancy,
          owner: item,
          parent,
          index,
        });
      ...
      if (top !== item && unresolved.indexOf(item) !== -1) {
        ...
        if (!intertwined) {
          ...
          const childIntertwined = traverse(
            ...
            0,
            [top],
            new Set<ItemContentDependancies>(),
            true
          );
  ...
  private identifyContentDependancies(
  ...
  public filterAny(
    action: (item: ItemContentDependancies) => boolean
  ): ItemContentDependancies[] {
